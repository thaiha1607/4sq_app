...
public class UserAddressGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all userAddresses").get("/api/user-addresses").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created userAddress").get("${new_userAddress_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created userAddress").delete("${new_userAddress_url}").headers(headers_http_authenticated))
                .pause(10)
        );
