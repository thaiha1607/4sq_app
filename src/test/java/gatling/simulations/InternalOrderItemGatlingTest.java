...
public class InternalOrderItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all internalOrderItems")
                    .get("/api/internal-order-items")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new internalOrderItem")
                        .post("/api/internal-order-items")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_internalOrderItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created internalOrderItem").get("${new_internalOrderItem_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created internalOrderItem").delete("${new_internalOrderItem_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the InternalOrderItem entity").exec(scn);
