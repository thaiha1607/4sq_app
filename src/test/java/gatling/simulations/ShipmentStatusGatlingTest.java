...
public class ShipmentStatusGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all shipmentStatuses").get("/api/shipment-statuses").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new shipmentStatus")
                        .post("/api/shipment-statuses")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_shipmentStatus_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created shipmentStatus").get("${new_shipmentStatus_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created shipmentStatus").delete("${new_shipmentStatus_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ShipmentStatus entity").exec(scn);
