...
public class ShipmentAssignmentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all shipmentAssignments")
                ...
                .exec(
                    http("Create new shipmentAssignment")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_shipmentAssignment_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created shipmentAssignment").get("${new_shipmentAssignment_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(http("Delete created shipmentAssignment").delete("${new_shipmentAssignment_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ShipmentAssignment entity").exec(scn);
